...
// Vertex shader code
//--------------------------------------------------------------------------------------------------
VS_OUTPUT vs(in float3 normal, in float3 tangent, in float3 binormal, in float2 texcoord, in float2 depth)
{
	//
	...
	Out.worldNormal.xyz = normalize(mul(normal, (float3x3)mWorldIT).xyz);

	//
	// Compute tangent in world space
	Out.worldTangent = normalize(mul(tangent, (float3x3)mWorldIT).xyz);
	...
	Out.worldBinormal = normalize(mul(binormal, (float3x3)mWorldIT).xyz);

	//
	// Forward texture coordinates
	Out.texcoord_depth.xy = texcoord;
...
{
	ISOLATE position = mul(float4(vIn.position0.xyz,1.0),  mWorldViewProjection);
	return vs(vIn.normal0, vIn.tangent0, vIn.binormal0, vIn.texcoord, position.zw);
}
