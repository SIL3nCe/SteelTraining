...
{
	float4 pos4 = float4(vIn.position0.xyz,1.0);
	ISOLATE position = mul(pos4, mWorldViewProjection);

	VS_OUTPUT Out;
	Out.texcoord.xy = vIn.texcoord;
	Out.worldPos = mul(pos4,mWorld).xyz;
	Out.shadtexcoord = mul(pos4, mShadowTexture);
